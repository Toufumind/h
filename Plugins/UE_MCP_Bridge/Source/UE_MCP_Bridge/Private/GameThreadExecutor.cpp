...
namespace
{
	...
	TSharedPtr<FJsonValue> MakeEditorInitializingError()
	{
...
TSharedPtr<FJsonValue> FMCPGameThreadExecutor::ExecuteOnGameThread(FHandlerFunction Handler, const TSharedPtr<FJsonObject>& Params, float TimeoutSeconds)
{
	if (IsGameThread())
	{
		if (!bEditorReady)
		{
			if (!IsEditorUsableForMCP())
			{
				return MakeEditorInitializingError();
			}
			SetEditorReady();
		}
	...
	FTSTicker::GetCoreTicker().AddTicker(
		FTickerDelegate::CreateLambda([this, State, Handler = MoveTemp(Handler), Params](float) -> bool
		{
			...
			if (!bEditorReady)
			{
				...
				{
					State->Result = MakeEditorInitializingError();
				}
			}
