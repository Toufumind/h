...
TSharedPtr<FJsonValue> FAnimationHandlers::AddPoseSearchSequence(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		FPoseSearchDatabaseBlendSpace NewEntry;
		NewEntry.BlendSpace = BlendSpace;
		Database->AddAnimationAsset(FInstancedStruct::Make(NewEntry));
	}
	else
	{
		return MCPError(FString::Printf(TEXT("Animation asset type not supported by this UE 5.6 bridge patch: %s"), *AnimAsset->GetClass()->GetName()));
	}
	Database->PostEditChange();
	UEditorAssetLibrary::SaveLoadedAsset(Database);
...
TSharedPtr<FJsonValue> FAnimationHandlers::ReadPoseSearchDatabase(const TSharedPtr<FJsonObject>& Params)
{
	...
	{
		const FPoseSearchDatabaseAnimationAssetBase* Entry = Database->GetDatabaseAnimationAsset<FPoseSearchDatabaseSequence>(i);
		if (!Entry) Entry = Database->GetDatabaseAnimationAsset<FPoseSearchDatabaseBlendSpace>(i);
		if (!Entry) Entry = Database->GetDatabaseAnimationAsset<FPoseSearchDatabaseAnimComposite>(i);
		if (!Entry) Entry = Database->GetDatabaseAnimationAsset<FPoseSearchDatabaseAnimMontage>(i);
		if (!Entry) Entry = Database->GetDatabaseAnimationAsset<FPoseSearchDatabaseMultiAnimAsset>(i);
		if (!Entry) continue;
		UObject* EntryAsset = Entry->GetAnimationAsset();
		TSharedPtr<FJsonObject> A = MakeShared<FJsonObject>();
		A->SetNumberField(TEXT("index"), i);
		A->SetStringField(TEXT("assetPath"), EntryAsset ? EntryAsset->GetPathName() : FString());
		A->SetStringField(TEXT("assetClass"), EntryAsset ? EntryAsset->GetClass()->GetName() : FString());
